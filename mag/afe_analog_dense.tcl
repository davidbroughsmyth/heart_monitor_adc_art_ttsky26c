...
# AZ caps mid-band (cy=20.5): above POSA (~14.5), below NEGB (gndB@23).
# X under NE art pocket once cell is placed at tile origin ~8.
set AZCY 20.5
afe::cap 24 24 185.0 $AZCY
afe::via2 199.0 $AZCY
afe::pbox met3 173.0 [expr {$AZCY-0.26}] 199.26 [expr {$AZCY+0.26}]
afe::via 199.0 $AZCY; afe::m1v 199.0 $AZCY [T az1]; afe::via 199.0 [T az1]
reg az1 199.0
afe::m4h $AZCY 173.0 201.0
afe::via3 171.0 $AZCY; afe::via2 171.0 $AZCY; afe::via 171.0 $AZCY
afe::m1v 171.0 $AZCY [T vp]; afe::via 171.0 [T vp]
reg vp 171.0

afe::cap 10 10 218.0 $AZCY
afe::via2 226.0 $AZCY
afe::pbox met3 213.0 [expr {$AZCY-0.26}] 226.26 [expr {$AZCY+0.26}]
afe::via 226.0 $AZCY; afe::m1v 226.0 $AZCY [T az2]; afe::via 226.0 [T az2]
reg az2 226.0
afe::m4h $AZCY 213.0 228.0
afe::via3 228.5 $AZCY; afe::via2 228.5 $AZCY; afe::via 228.5 $AZCY
afe::m1v 228.5 $AZCY [T vm]; afe::via 228.5 [T vm]
reg vm 228.5
...
# ===== Row A: CM + AZ — east of DAC-A (~ends x≈208), under art X (~210+) =====
set X 212.0
...
set JOGS {{gnd gndB 260.0} {vdd vddB 262.5} {vref vrefB 265.0} \
          {n3 n3b 267.5} {dac_out vdac 270.0}}
